...
{
    uint32 *length = (uint32*)location;
    *length = sizeof(ParserFunc) * PARSER_STATE_SIZE;

    byte *start = (byte*)length + 1;
    byte *end = location + *length;

    clearMemory((uint32*)start, (uint32*)end);

    ParserFunc *parserState = (ParserFunc*)start;

    parserState[PARSER_SEARCHING] = &parser_searching;
}

void parser_searching(ParserLocals *arg)
{
...
void loadBuffer(LoadBufferOptions *opts)
{
    /* calculate result pointer offsets */
    LoadBufferResult *result = (LoadBufferResult*)(opts + 1);
    LoadBufferStats *stats = &result->stats;
    LoadBufferLocals *self = (LoadBufferLocals*)(result + 1);
    ColumnDescriptor *columns = (ColumnDescriptor*)(self + 1);
    byte *data = (byte*)(columns + opts->columnCount);
    byte *rows = data + opts->length;

    /* clear the values (there might be garbage in them) */
    clearMemory((uint32*)result, (uint32*)data);
